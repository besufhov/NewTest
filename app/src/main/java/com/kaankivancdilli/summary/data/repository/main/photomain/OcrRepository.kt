package com.kaankivancdilli.summary.data.repository.main.photomain
...
class OcrRepository @Inject constructor(
    ...
    suspend fun processUriWithTilingSafe(
        uri: Uri,
        maxTileSize: Int = 1024,
        maxDim: Int = 2000,
        maxPixels: Long = 8_000_000,
        maxFileSizeMB: Int = 12
        ...
        try {
            ...
            if (origW <= 0 || origH <= 0) return ResultStateOCR.Error(corruptedMsg) to ResultStateOCR.Error(
                corruptedMsg
            )
            ...
            val textResult = if (finalText.isBlank()) ResultStateOCR.Error("OCR skipped (empty)") else ResultStateOCR.Success(
                finalText
            )
